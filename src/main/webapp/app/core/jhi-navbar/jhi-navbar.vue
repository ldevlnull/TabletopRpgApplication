    ...
    <b-navbar toggleable="md" type="dark" class="jh-navbar">
        ...
        <b-collapse is-nav id="header-tabs">
            <b-navbar-nav class="ml-auto">
                <b-nav-item v-if="hasAnyAuthority('ROLE_MODERATOR')" to="/admin/user-management">
                    <font-awesome-icon icon="user"/>
                    <span v-text="$t('global.menu.admin.userManagement')">User management</span>
                </b-nav-item>
                <b-nav-item v-if="hasAnyAuthority('ROLE_MODERATOR')" to="/game-system">
                    <font-awesome-icon icon="asterisk"/>
                    <span v-text="$t('global.menu.entities.gameSystem')">Game Systems</span>
                </b-nav-item>
                <b-nav-item v-if="hasAnyAuthority('ROLE_MODERATOR')" to="/game-tag">
                    <font-awesome-icon icon="asterisk"/>
                    <span v-text="$t('global.menu.entities.gameTag')">Game Tags</span>
                </b-nav-item>
                <b-nav-item v-if="authenticated" to="/character">
                    <font-awesome-icon icon="asterisk"/>
                    <span v-text="$t('global.menu.entities.character')">My Character</span>
                </b-nav-item>
                <b-nav-item-dropdown
                    id="admin-menu"
                ...
                <b-nav-item v-if="!authenticated" v-on:click="openLogin()" id="login">
                    <font-awesome-icon icon="sign-in-alt"/>
                    <span v-text="$t('global.menu.account.login')">Sign in</span>
                </b-nav-item>
                <b-nav-item to="/register" tag="b-dropdown-item" id="register" v-if="!authenticated">
                    <font-awesome-icon icon="user-plus"/>
...
<style scoped>
    ...
    */
    ...
    .logo-img {
        height: 100%;
        background: url("../../../content/images/trpg-logo.png") no-repeat center center;
