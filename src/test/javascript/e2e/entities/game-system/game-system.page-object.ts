...
export default class GameSystemComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('game-system-heading'));
...
export class GameSystemDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('trpgPlanningApplicationApp.gameSystem.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-gameSystem'));
}
